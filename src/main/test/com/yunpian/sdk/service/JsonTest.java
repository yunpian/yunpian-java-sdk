	...
	public void test() {
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			for (int j = 0; j < jsonObject.get("cities").getAsJsonArray().size(); j++) {
				...
				JsonObject jsonObject1 = jsonObject.get("cities").getAsJsonArray().get(j).getAsJsonObject();
				...
				pro.addProperty(jsonObject1.get("lable").getAsString(), JsonUtil.toJson(jsonObject1.get("counties")));
